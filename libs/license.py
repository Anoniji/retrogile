    ...
    def validate_license(self, type_lst = []):
        ...
        try:
            ...
            if (
                ...
                not jwt_decoded.get('permits', False) or
                not isinstance(jwt_decoded.get('permits'), list)
            ):
                print("Error: Schema verification failed")
                sys.exit(1)

            # gen_datetime = datetime.fromisoformat(jwt_data.get('last_accessed_at').replace('Z', '+00:00'))
